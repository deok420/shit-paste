...
bool weapon_t::can_double_tap()
{
	...
	if (idx == WEAPON_TASER || idx == WEAPON_SSG08 || idx == WEAPON_REVOLVER || idx == WEAPON_AWP || idx == WEAPON_XM1014 || idx == WEAPON_NOVA || idx == WEAPON_SAWEDOFF || idx == WEAPON_MAG7)
		return false;
...
//
//	if (idx == WEAPON_REVOLVER || idx == WEAPON_SSG08 || idx == WEAPON_AWP || idx == WEAPON_XM1014 || idx == WEAPON_NOVA || idx == WEAPON_SAWEDOFF || idx == WEAPON_MAG7)
...
void player_t::InitiateAnimation(int layer, int activity)
{
	int sequence;

	bool m_bisCrouching = get_animation_state()->m_fDuckAmount > 0.55f;
	auto m_flRunningSpeed = get_animation_state()->m_flFeetSpeedForwardsOrSideWays;
	bool m_bMoving = m_flRunningSpeed > 0.25f;
	switch (activity)
	{
	case 989:                                   // ACT_CSGO_LAND_HEAVY
		sequence = 23;
		if (m_bisCrouching)
			sequence = 24;
		break;
	case 986:                                   // ACT_CSGO_FALL
		sequence = 14;
		break;
	case 985:                                   // ACT_CSGO_JUMP
		sequence = m_bMoving + 17;
		if (!m_bisCrouching)
			sequence = m_bMoving + 15;
		break;
	case 987:                                   // ACT_CSGO_CLIMB_LADDER
		sequence = 13;
		break;
	case 988:                                   // ACT_CSGO_LAND_LIGHT
		sequence = 2 * m_bMoving + 20;
		if (m_bisCrouching)
		{
			sequence = 21;
	...
	get_animlayers()[layer].m_flPlaybackRate = GetLayerSequenceCycleRate(&get_animlayers()[layer], sequence);
	get_animlayers()[layer].m_nSequence = sequence;
	get_animlayers()[layer].m_flCycle = get_animlayers()[layer].m_flWeight = 0.f;
}
...
{
	static auto m_uInvalidatePhysics = util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 83 E4 F8 83 EC 0C 53 8B 5D 08 8B C3"));
