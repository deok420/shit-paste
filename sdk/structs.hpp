...
public:
    ...
    //datamap_t(uint32_t, get_buttons, crypt_str("CBaseCombatCharacter"), crypt_str("m_nButtons"));
    //datamap_t(uint32_t, get_buttons_last, crypt_str("CBaseCombatCharacter"), crypt_str("m_afButtonLast"));
    //datamap_t(uint32_t, get_buttons_pressed, crypt_str("CBaseCombatCharacter"), crypt_str("m_afButtonPressed"));
    //datamap_t(uint32_t, get_buttons_released, crypt_str("CBaseCombatCharacter"), crypt_str("m_afButtonReleased"));
	...
	void SetCurrentCommand(CUserCmd*);
    ...
    float GetLayerSequenceCycleRate(AnimationLayer* state, int32_t layer) {
        return VCall< float(__thiscall*)(void*, AnimationLayer*, int32_t)>(this, 222)(this, state, layer);
    }
    ...
    uint32_t& m_fEffects();
    uint32_t& m_iEFlags();
    void InitiateAnimation(int layer, int activity);
...
struct CPacketManager
{
	int m_nSequence = 0;
	int	m_nLastSequence = 0;
	CUserCmd* m_aCommands;
	bool m_bResetModifablePacket = 0;
	BYTE gap339D[7];
	int* pint33A4 = 0;
	int* pint33A8 = 0;
};
...
struct CCSGOPlayerAnimationState_534535_t
{
public:
    char        pad0[0x60]; // 0x00
    void* pEntity; // 0x60
