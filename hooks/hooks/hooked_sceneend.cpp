...
void __fastcall hooks::hooked_sceneend(void* ecx, void* edx)
{
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
	{
		...
		{
			m_materialsystem()->FindMaterial(crypt_str("supremacy.idb_chams"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("supremacy.idb_metallic"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("supremacy.idb_flat"), nullptr),
			...
			m_materialsystem()->FindMaterial(crypt_str("supremacy.idb_glow"), nullptr)
		};
