...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("player_death")))
	{
		...
		if (g_ctx.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer())
		{
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			float flEraseTime = g_ctx.local()->m_nTickBase() * m_globals()->m_intervalpertick + 3.f;
			...
			if (weapon_is_aim(weapon))
			{
				...
				otheresp::get().damage_marker[user_id] = otheresp::Damage_marker
				{
					...
					(0.1f * (CurrentTime - flEraseTime)),
					m_globals()->m_curtime,
					g_cfg.esp.damage_marker_color,
					damage,
					hitgroup
				};
			}
			...
			if (current_shot)
			{
				...
				if (m_engine()->GetPlayerInfo(current_shot->last_target, &player_info) && !player_info.fakeplayer)
				{
					...
					for (auto it = lagcompensation::get().player_sets.begin(); it != lagcompensation::get().player_sets.end(); ++it)
						// after finding we need to compare server and client hitbox
						if (it->id == player_info.steamID64 && strstr(current_shot->shot_info.server_hitbox.c_str(), current_shot->shot_info.client_hitbox.c_str()))
						{
							// did we hit head?
							...
							if (((resolver_side)current_shot->side == RESOLVER_FIRST || (resolver_side)current_shot->side == RESOLVER_SECOND))
							{
								if (is_head)
								{
									...
									if (current_shot->shot_info.speed > 38.f)
										it->low_move = false;
									else
										it->low_stand = false;
								}

								// increase amount of hitted side
