...
//bool __fastcall Hooks::hkInPrediction(void* m_pPrediction)
//{
//	// [COLLAPSED LOCAL DECLARATIONS. PRESS KEYPAD CTRL-"+" TO EXPAND]
//
//	local_player_index = g_iLocalPlayer;
//	Game::CallVFunction(g_pEngineClient, 12);
//	m_pLocalWrapper = m_fnGetLocalPlayer();
//	m_fnGetClientEntity = EngineClient::GetClientEntity(local_player_index, m_pLocalWrapper);
//	if (!m_fnGetClientEntity(v6))               // if ( !g_LocalPlayer )
//		return Hooks::o_InPrediction(m_pPrediction, 0);
//	result = Hooks::o_InPrediction(m_pPrediction, 0);
//	if (byte_4A66AC28)
//	{
//		if (retaddr == dword_4AB2E514)
//			result = 0;
//	}
//	return result;
//}
...
bool __fastcall hooks::hooked_writeusercmddeltatobuffer(void* ecx, void* edx, int slot, bf_write* buf, int from, int to, bool is_new_command)
{
	...
	*backup_commands = 0;
	...
	//auto choked_modifier = min(newcmds + abs(shift), 62);
...
next_cmd:
	...
	to_cmd.m_tickcount += 2 * int(1 / m_globals()->m_intervalpertick);

	//for (auto i = choked_modifier - newcmds + 1; i > 0; --i)
	for (auto i = newcmds; i <= choked_modifier; i++)
	{
