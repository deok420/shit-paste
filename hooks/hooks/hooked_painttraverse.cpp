...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (reload_fonts())
	{
		...
		fonts[ESP] = create_font(crypt_str("smallfonts"), 8, 500, FONTFLAG_ANTIALIAS | FONTFLAG_OUTLINE);
		fonts[NAME] = create_font(crypt_str("Tahoma"), 12, 400, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		...
		fonts[WARNING] = create_font(crypt_str("calibri"), 35, FW_HEAVY, BOLD_FONTTYPE);
		...
		fonts[WEAPONICONS] = create_font(crypt_str("Counter-Strike"), 22, 400, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[INDICATORFONT1] = create_font(crypt_str("calibri"), 26 , 600, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		...
		fonts[WARNING1] = create_font(crypt_str("Verdana"), 8, 500, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		...
		if (g_ctx.available())
		{
			...
			if (g_cfg.player.enable && g_cfg.esp.removals[REMOVALS_SCOPE] && g_ctx.globals.scoped && weapon->is_sniper())
			{
				...
				if (g_cfg.esp.fade_scope) {
					render::get().gradient(cx - g_cfg.esp.gap_sniper - 20 - 60 - g_cfg.esp.size_sniper, cy, 60 + g_cfg.esp.size_sniper, 1, Color(g_cfg.esp.fade2), Color(g_cfg.esp.fade1), GRADIENT_HORIZONTAL);
					render::get().gradient(cx + g_cfg.esp.gap_sniper + 20, cy, 60 + g_cfg.esp.size_sniper, 1, Color(g_cfg.esp.fade1), Color(g_cfg.esp.fade2), GRADIENT_HORIZONTAL);

					render::get().gradient(cx, cy - g_cfg.esp.gap_sniper - 20 - 60 - g_cfg.esp.size_sniper, 1, 60 + g_cfg.esp.size_sniper, Color(g_cfg.esp.fade2), Color(g_cfg.esp.fade1), GRADIENT_VERTICAL);
					render::get().gradient(cx, cy + g_cfg.esp.gap_sniper + 20, 1, 60 + g_cfg.esp.size_sniper, Color(g_cfg.esp.fade1), Color(g_cfg.esp.fade2), GRADIENT_VERTICAL);
				}
				else {
...
//				alpha -= speed;
//
//			ren*/der::get().rect_filled(0, 0, w, h, Color(0, 0, 0, alpha));
//		}
