...
#include "../gilroy-bold.h"
#include "../gilroy-light.h"
...
namespace hooks
{
	...
	using shit = void(__thiscall*)(void*, void*, int, int, int, int);

	void __fastcall hkPerformScreenOverlay(void* _this, void* edx, int x, int y, int w, int h) {
		if (g_cfg.misc.remove_ad)
			return;

		static auto ofunc = (shit)(hooks::original_performscreenoverlay);
	...
	void GUI_Init(IDirect3DDevice9* pDevice)
	{
		...
		static auto impact_directory = (std::string)windows_directory + "\Fonts\Impact.ttf";
		...
		if (c_menu::get().bg == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(pDevice, &backgroun1d, sizeof(backgroun1d),
				670, 412,
				D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT,
				D3DX_DEFAULT, D3DX_DEFAULT, NULL,
				(D3DXIMAGE_INFO*)NULL, (PALETTEENTRY*)NULL, &c_menu::get().bg);
		}
		if (c_menu::get().childBg == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(pDevice, &childImage, sizeof(childImage),
				220, 350,
				D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT,
				D3DX_DEFAULT, D3DX_DEFAULT, NULL,
				(D3DXIMAGE_INFO*)NULL, (PALETTEENTRY*)NULL, &c_menu::get().childBg);
		}
		if (c_menu::get().check == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(pDevice, &checkmark, sizeof(checkmark),
				24, 25,
				D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT,
				D3DX_DEFAULT, D3DX_DEFAULT, NULL,
				(D3DXIMAGE_INFO*)NULL, (PALETTEENTRY*)NULL, &c_menu::get().check);
		}

		io.Fonts->AddFontFromMemoryTTF(gilroymedium, sizeof(gilroymedium), 14.0f);
		c_menu::get().Title = io.Fonts->AddFontFromMemoryTTF(gilroybold, sizeof(gilroybold), 20.0f);
		c_menu::get().child = io.Fonts->AddFontFromMemoryTTF(gilroymedium, sizeof(gilroymedium), 13.0f);
		c_menu::get().tab_icon = io.Fonts->AddFontFromMemoryTTF(teslafont, sizeof(teslafont), 18.0f);
		c_menu::get().elements = io.Fonts->AddFontFromMemoryTTF(teslafont, sizeof(teslafont), 22.0f);
		c_menu::get().elements_mini = io.Fonts->AddFontFromMemoryTTF(teslafont, sizeof(teslafont), 20.0f);
		c_menu::get().Logo = io.Fonts->AddFontFromMemoryTTF(teslafont, sizeof(teslafont), 30.0f);
/*
		c_menu::get().tabs = io.Fonts->AddFontFromMemoryTTF(gilroymedium, sizeof(gilroymedium), 14.0f);
		c_menu::get().cheatName = io.Fonts->AddFontFromMemoryTTF(gilroybold, sizeof(gilroybold), 14.0f);
		c_menu::get().cheatVer = io.Fonts->AddFontFromMemoryTTF(gilroylight, sizeof(gilroylight), 12.0f);*/

		//c_menu::get().futura_small = io.Fonts->AddFontFromMemoryTTF((void*)futuramediumc, sizeof(futuramediumc), 15.f, &m_config, ranges);
		c_menu::get().futura_small = io.Fonts->AddFontFromFileTTF(verdana_directory.c_str(), 15.f, &m_config, ranges);
		//c_menu::get().futura = io.Fonts->AddFontFromMemoryTTF((void*)futuramediumc, sizeof(futuramediumc), 17.f, &m_config, ranges);
		c_menu::get().futura = io.Fonts->AddFontFromFileTTF(verdana_directory.c_str(), 17.f, &m_config, ranges);
		//c_menu::get().futura_large = io.Fonts->AddFontFromMemoryTTF((void*)futuramediumc, sizeof(futuramediumc), 20.f, &m_config, ranges);
		c_menu::get().futura_large = io.Fonts->AddFontFromFileTTF(verdana_directory.c_str(), 20.f, &m_config, ranges);

		c_menu::get().gotham = io.Fonts->AddFontFromMemoryTTF((void*)GothamPro, sizeof(GothamPro), 13.f, &m_config, ranges);

		// Icon fonts
		c_menu::get().ico_sk = io.Fonts->AddFontFromMemoryTTF((void*)ico_skeet, sizeof(ico_skeet), 23.f, &m_config, io.Fonts->GetGlyphRangesCyrillic());
		c_menu::get().ico_menu = io.Fonts->AddFontFromMemoryTTF((void*)icomenu, sizeof(icomenu), 20.f, &m_config, io.Fonts->GetGlyphRangesCyrillic());
		c_menu::get().ico_bottom = io.Fonts->AddFontFromMemoryTTF((void*)iconbot, sizeof(iconbot), 20.f, &m_config, io.Fonts->GetGlyphRangesCyrillic());

		c_menu::get().impact = io.Fonts->AddFontFromFileTTF(impact_directory.c_str(), 20.f, &m_config, io.Fonts->GetGlyphRangesCyrillic());
