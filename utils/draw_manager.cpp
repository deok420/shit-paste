...
void render::circle(int x, int y, int points, int radius, Color color)
{
	...
	m_surface()->DrawSetColor(color);
	m_surface()->DrawOutlinedCircle(x, y, radius, points);
}

void render::Draw3DCircleGradient(const Vector& origin, float radius, Color color, Color color2)
{
	...
	for (int i = 1; i < radius; i++) {
		if (rad > 2.f)
			Draw3DFilledCircleMono(origin, round(rad), Color(color.r(), color.g(), color.b(), i));

		rad -= 1.f;
	}
}

void render::Draw3DFilledCircleMono(const Vector& origin, float radius, Color color)
{
	static auto prevScreenPos = ZERO; //-V656
...
void render::circle_filled(int x, int y, int points, int radius, Color color)
{
	...
	std::vector<float> temppointsx;
	std::vector<float> temppointsy;
...
void render::Draw3DCircleNoResize(const Vector& origin, float radius, Color color)
{
	auto prevScreenPos = ZERO;
	auto step = M_PI * 2.0f / 72.0f;
