...
namespace util
{
	...
	bool is_breakable_entity(IClientEntity* e)
	{
		...
		static auto is_breakable_fn = reinterpret_cast<bool(__thiscall*)(IClientEntity*)>(
			util::FindSignature(crypt_str("client.dll"), crypt_str("55 8B EC 51 56 8B F1 85 F6 74 68")));
	...
	bool get_bbox(entity_t* e, Box& box, bool player_esp)
	{
		if (!player_esp) {
			...
			Vector w2sBottom, w2sTop;

			math::world_to_screen(e->m_vecOrigin() - Vector(0, 0, 4), w2sBottom);
			math::world_to_screen(reinterpret_cast<player_t*>(e)->hitbox_position(0) + Vector(0, 0, 8), w2sTop);

			int Middle = w2sBottom.y - w2sTop.y;
			int Width = Middle / 3.75f;

			box.x = w2sBottom.x - Width; // left
			box.y = w2sTop.y; // top
			box.w = Width * 2; // right
			box.h = Middle; // bottom
		}
