...
void C_ConfigManager::setup()
{
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].autoscope_distance, false, crypt_str("Ragebot.autoscope_distance"));
		setup_item(&g_cfg.ragebot.weapon[i].autoscope_distance_float, 0, crypt_str("Ragebot.autoscope_distance_float"));
	...
	setup_item(&g_cfg.antiaim.static_legs_in_air, false, crypt_str("Antiaim.static_legs_in_air"));
	setup_item(&g_cfg.antiaim.pitch0land, false, crypt_str("Antiaim.pitch0land"));
	...
	setup_item(&g_cfg.antiaim.desync_dir, 0, crypt_str("Antiaim.desync_dir"));
	...
	setup_item(&g_cfg.esp.grenade_prediction, false, crypt_str("Esp.grenade_prediction"));//g_cfg.esp.grenadetracer1
	setup_item(&g_cfg.esp.on_click , false, crypt_str("Esp.on_click"));
	setup_item(&g_cfg.esp.grenadetracer1, false, crypt_str("Esp.grenade_warning"));
	...
	setup_item(&g_cfg.esp.keybinds, Color(255, 255, 255), crypt_str("Esp.keybinds"));
	setup_item(&g_cfg.esp.spectatorss, Color(255, 255, 255), crypt_str("Esp.spectatorss"));
	setup_item(&g_cfg.esp.fakeindic, Color(255, 255, 255), crypt_str("Esp.fakeindic"));//watermark_color
	setup_item(&g_cfg.misc.watermark_color, Color(255, 255, 255), crypt_str("Esp.watermark_color"));
	...
	setup_item(&g_cfg.misc.strafe_smoothing, 0, crypt_str("Misc.strafe_smoothing"));
	...
	setup_item(&g_cfg.misc.automatic_peek2, Color(255, 255, 255), crypt_str("automatic_peek_color"));
	...
	setup_item(&g_cfg.misc.retrack_speed, 0.f, crypt_str("Misc.retrack_speed"));
	...
	setup_item(&g_cfg.misc.centerinds, false, crypt_str("Misc.Centerinds"));

	setup_item(&g_cfg.misc.centerinds_active, Color(0, 0, 0), crypt_str("Misc.centerinds_active_color"));
	setup_item(&g_cfg.misc.centerinds_inactive, Color(0, 0, 0), crypt_str("Misc.centerinds_inactive_color"));

	setup_item(&g_cfg.esp.fade1, Color(0, 0, 0), crypt_str("Esp.fade1"));
	setup_item(&g_cfg.esp.fade2, Color(0, 0, 0), crypt_str("Esp.fade2"));

	setup_item(&g_cfg.misc.keybinds, false, crypt_str("Misc.Keybinds"));
	setup_item(&g_cfg.misc.keybinds1, false, crypt_str("Misc.Fakeind"));
	setup_item(&g_cfg.misc.InfoBar, false, crypt_str("Misc.InfoBar"));
	setup_item(&g_cfg.misc.evoinds, false, crypt_str("Misc.Someindicators"));
	setup_item(&g_cfg.misc.fakeline, false, crypt_str("Misc.Fakeline"));

	setup_item(&g_cfg.esp.fade_scope, false, crypt_str("Esp.FadeScope"));

	setup_item(&g_cfg.esp.taser_range, false, crypt_str("Esp.taser_range"));

	setup_item(&g_cfg.esp.trails, false, crypt_str("Esp.taser_range"));
	setup_item(&g_cfg.esp.velocity_graph, false, crypt_str("Esp.velocity_graph"));

	setup_item(&g_cfg.esp.grenadetracer, false, crypt_str("Esp.GrenadeTrails"));
	setup_item(&g_cfg.esp.advance_mode, false, crypt_str("Esp.AdvancedMode"));

	setup_item(&g_cfg.esp.grenadetrace, Color(0, 0, 0), crypt_str("Esp.GrenadeTrailscolor"));

	setup_item(&g_cfg.esp.dist, 0.0f, crypt_str("Esp.DistanceLimit"));

	setup_item(&g_cfg.esp.size_sniper, 0.0f, crypt_str("Esp.FadeScope_Size"));
	setup_item(&g_cfg.esp.gap_sniper, 0.0f, crypt_str("Esp.FadeScope_Gap"));

	setup_item(&g_cfg.misc.fakeline_color, Color(0, 0, 0), crypt_str("Misc.fakeline_color"));

	setup_item(&g_cfg.esp.helper, false, crypt_str("Misc.Grenadehelper"));

	setup_item(&g_cfg.esp.circlefake, false, crypt_str("Misc.CircleDesyncIndicator"));

	setup_item(&g_cfg.esp.circlefake_color, Color(255, 255, 255), crypt_str("Misc.CircleDesyncIndicator_Color"));

	setup_item(&g_cfg.esp.helper_color, Color(0, 0, 0), crypt_str("Misc.helpercolor"));

	setup_item(&g_cfg.esp.helper_distance, 0.0f, crypt_str("Misc.Helper_Distance"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\ExodiumFrez\\Configs\\") + config;
		}
	};
